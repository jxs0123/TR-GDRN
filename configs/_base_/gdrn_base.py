...
_base_ = "./common_base.py"
# ----------------------------------------------------------------------------- 
# base model cfg for gdrn + convnext backbone
# -----------------------------------------------------------------------------
MODEL = dict(
    DEVICE="cuda",
    WEIGHTS="",
    ...
    CDPN=dict(
        ...
        ## backbone
        BACKBONE=dict(
            PRETRAINED="",  # timm pretrained
            ARCH="convnext",        # 支持 convnext
            VARIANT="convnext_tiny.in12k_ft_in1k",       # tiny | small | base | large
            INPUT_CHANNEL=3,
            INPUT_RES=256,
            OUTPUT_RES=64,
            OUT_CHANNELS=768,       # head 接收通道数
            FREEZE=False,
            USE_TRANSFORMER=True,  # transformer block
            USE_ATTENTION=False,    # eca attention 
            ...
            ATTENTION_LW=0.0,
            ATTENTION_REDUCTION='mean',  # mean | sum
        ),
        ## rot head
        ROT_HEAD=dict(
            USE_AGA=False,      # 启用 AGA（可选）
            USE_HTD=True,     # 启用 HTD（你现在想要的）
            HTD_NUM_LAYERS=1,  # HTD decoder 层数
            HTD_NHEAD=1,       # attention heads
            HTD_FF=768,       # FFN 隐层维度
        ...
        ## direct regression PnP
        PNP_NET=dict(
            FREEZE=False,
            R_ONLY=False,
            LR_MULT=1.0,
            PNP_HEAD_CFG=dict(type="ConvPnPNet", norm="GN", num_gn_groups=32, drop_prob=0.0),
            WITH_2D_COORD=False,
