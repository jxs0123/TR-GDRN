...
from .convnext_backbone import ConvNeXtBackboneNet # NEW
...
def build_model_optimizer(cfg):
    ...
    # Helper: small wrapper to apply a 1x1 reduce on the final feature map only
    class _BackboneWithReduce(nn.Module):
        def __init__(self, backbone: nn.Module, reduce: nn.Module, rot_concat: bool):
            super().__init__()
            self.backbone = backbone
            self.reduce = reduce
    ...
    if "resnet" in backbone_cfg.ARCH:
        ...
        backbone_net = ConvNeXtBackboneNet(
            variant=variant,
            in_channel=backbone_cfg.INPUT_CHANNEL,
            pretrained=True,
            freeze=backbone_cfg.FREEZE,
            rot_concat=r_head_cfg.ROT_CONCAT,
    ...
    # --------------------------
    # Align backbone channels to heads
    # --------------------------
    TARGET_IN_CHANNELS = getattr(backbone_cfg, "OUT_CHANNELS", 512)
    if last_channels != TARGET_IN_CHANNELS:
        backbone_reduce = nn.Conv2d(last_channels, TARGET_IN_CHANNELS, kernel_size=1, bias=False)
        # init like Kaiming normal is fine; BN/GN handled in head
        params_lr_list.append({"params": backbone_reduce.parameters(), "lr": float(cfg.SOLVER.BASE_LR)})
    else:
        backbone_reduce = nn.Identity()

    # Wrap so that only the final feature is reduced; skip low-level features
    backbone_net = _BackboneWithReduce(backbone_net, backbone_reduce, r_head_cfg.ROT_CONCAT)
    ...
    if not t_head_cfg.ENABLED:
        ...
        if t_head_cfg.FREEZE:
            for p in trans_head_net.parameters():
                p.requires_grad = False
        else:
            params_lr_list.append({
    ...
    # --------------------------
    # PnP head build (unchanged except for computed dims)
    # --------------------------
    ...
    pnp_head_cfg = dict(pnp_net_cfg.PNP_HEAD_CFG)  # shallow copy
